...
def liquidity_trader_order(simulation_id, stock_id):
    ...
    open_quantity = Order.objects.all().filter(stock_id=stock.id, order_type=second_order_type, price__isnull=False, quantity_lt=50).aggregate(Sum('quantity'))['quantity__sum']
    if open_quantity:
    ...
    open_quantity = Order.objects.all().filter(stock_id=stock.id, order_type=first_order_type, price__isnull=False, quantity_lt=50).aggregate(Sum('quantity'))['quantity__sum']
    if open_quantity:
        quantity = randint(0, int(open_quantity*0.15))
        if quantity > 0:
